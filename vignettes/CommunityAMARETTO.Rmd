...
the system and facilitating biomarker discovery.
...
functional enrichment of detected communities and to generate interactive
...
liver samples [3]. These sample AMARETTO-result objects can be accessed through
the CommunityAMARETTO package as shown below, and are derived from a small
portion of the real data, therefore are for representation only. 
...
This function construct a network with its nodes each representing a module.
...
between the union of genes inside the community against gensets of our interest
...
to feed it to the html report. However it is potional and
