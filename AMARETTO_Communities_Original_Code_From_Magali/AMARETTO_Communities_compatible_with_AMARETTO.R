...
############################################
#' @param a list of multiple AMARETTO results
#' @param a p-value threshold for connecting modules via edges
#' @param a p-value threshold for drawing edges
#'
#' @return a list with a p-value matrix, edge matrix, and graph layout 
#'
#' @author Magali Champion, Rileen Sinha (rileen@gmail.com)
#'
#' @concept AMARETTO
#' @export
...
AMARETTO_Pancancer <- function(AMARETTOPancancerData,InnerThreshold=c(pvalue=0.05,inter=5),Threshold=0.05){
    ...
    for (i in (1:length(AMARETTOPancancerData))){
      GeneNames <- c(GeneNames,AMARETTOPancancerData[[i]]["AllGenes"])
    }  
    ...
    NrClusters <- c()
    for (i in (1:length(AMARETTOPancancerData))){
      NrClusters <- c(NrClusters,AMARETTOPancancerData[[i]]["NrModules"])
    }
    names(NrClusters) <- names(AMARETTOPancancerData)
    N <- sum(unlist(NrClusters))
   
    Assign <- c()
    for (i in (1:length(AMARETTOPancancerData))){
      Assign <- c(Assign,AMARETTOPancancerData[[i]]["ModuleMembership"])
    }
    ...
    graph.degrees <- igraph::degree(ModuleGraph)
...
####################################################
#' @param a list with a p-value matrix, edge matrix, and graph layout
#' @param a threshold for ratio of community size to comunity network size
#' @param a threshold for minimum number of cancers/diseases in a community
#' @param a threshold for ratio of incoming to outgoing edges
#'
#' @return a list with a p-value matrix, edge matrix, and graph layout 
#'
#' @author Magali Champion, Rileen Sinha (rileen@gmail.com)
...
FindRegulators <- function(CommData,Community,AMARETTOPancancerData){
    ...
    for (i in (1:length(ModuleName))){
        ...
        ModuleNr <- strsplit2(myData, "_")[,1]
        ModuleNr <- as.numeric(str_replace_all(ModuleNr,"[[A-Z]]","")) 
        res <- AMARETTOPancancerData[[Cancer]]["RegulatoryPrograms"]
        I = which(abs(res$RegulatoryPrograms[ModuleNr,])>0)
